...
void loop() {
  ...
  if(teclaPrecionada){//se crea la condiciÃ³n
    ...
    if(strcmp(pass,ingresaPass)==0){ //Verifica que la contraseÃ±a esta correcta
      pantallalcd.setCursor(0,1);//se coloque en en la columna cero y se imprima en la fila
      pantallalcd.print("abrir las puertas");
      delay(2000);
      ...
      pantallalcd.print("bienvenido pa");
      delay(5000);
      pantallalcd.clear();//luego que pase el tiempo se limpie
      pantallalcd.setCursor(1,1);
      pantallalcd.print("cerrara en 10s");
      delay(10000);
      pantallalcd.clear();
      dota.write(0);
      delay(2000);
      pantallalcd.setCursor(1,1);
      pantallalcd.print("puerta cerrada");
      delay(1000);
      pantallalcd.setCursor(1,1);
      pantallalcd.print("cerrado");
      pantallalcd.clear();           
      ...
      pantallalcd.print("acceso no permitido");
      delay(3000);
      pantallalcd.clear();//luego que pase el tiempo se limpie  
    }indice=0;
  }
